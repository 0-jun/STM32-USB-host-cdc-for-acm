...
static USBH_StatusTypeDef SetControlLineState(USBH_HandleTypeDef *phost, uint16_t control_line_state);
...
USB_CDC_CLASS, USBH_CDC_InterfaceInit, USBH_CDC_InterfaceDeInit, USBH_CDC_ClassRequest, USBH_CDC_Process, USBH_CDC_SOFProcess,
NULL, };
/**
 * @}
 */
...
static USBH_StatusTypeDef USBH_CDC_InterfaceInit(USBH_HandleTypeDef *phost) {
	...
	(void) USBH_OpenPipe(phost, CDC_Handle->CommItf.NotifPipe, CDC_Handle->CommItf.NotifEp, phost->device.address, phost->device.speed, USB_EP_TYPE_INTR, CDC_Handle->CommItf.NotifEpSize);
	...
	(void) USBH_OpenPipe(phost, CDC_Handle->DataItf.OutPipe, CDC_Handle->DataItf.OutEp, phost->device.address, phost->device.speed, USB_EP_TYPE_BULK, CDC_Handle->DataItf.OutEpSize);

	/* Open channel for IN endpoint */
	(void) USBH_OpenPipe(phost, CDC_Handle->DataItf.InPipe, CDC_Handle->DataItf.InEp, phost->device.address, phost->device.speed, USB_EP_TYPE_BULK, CDC_Handle->DataItf.InEpSize);
...
static USBH_StatusTypeDef USBH_CDC_Process(USBH_HandleTypeDef *phost) {
	...
	case CDC_GET_LAST_LINE_CODING_STATE:
		...
		if (req_status == USBH_OK) {
			...
			if ((CDC_Handle->LineCoding.b.bCharFormat == CDC_Handle->pUserLineCoding->b.bCharFormat) && (CDC_Handle->LineCoding.b.bDataBits == CDC_Handle->pUserLineCoding->b.bDataBits) && (CDC_Handle->LineCoding.b.bParityType == CDC_Handle->pUserLineCoding->b.bParityType)
...
/**
 * @brief  This request applies to set dts and rts
 * @param  phost: Selected device
 * @param  dtsrts: state of dtsrts
 * @retval USBH_StatusTypeDef : USB ctl xfer status
 */
static USBH_StatusTypeDef SetControlLineState(USBH_HandleTypeDef *phost, uint16_t dtsrts) {
	phost->Control.setup.b.bmRequestType = USB_H2D | USB_REQ_TYPE_CLASS | USB_REQ_RECIPIENT_INTERFACE;
	phost->Control.setup.b.bRequest = CDC_SET_CONTROL_LINE_STATE;
	phost->Control.setup.b.wValue.w = dtsrts;
	phost->Control.setup.b.wIndex.w = 0U;
...
static void CDC_ProcessTransmission(USBH_HandleTypeDef *phost) {
	...
	case CDC_SEND_DATA:
		if (CDC_Handle->TxDataLength > CDC_Handle->DataItf.OutEpSize) {
			(void) USBH_BulkSendData(phost, CDC_Handle->pTxData, CDC_Handle->DataItf.OutEpSize, CDC_Handle->DataItf.OutPipe, 1U);
		} else {
			(void) USBH_BulkSendData(phost, CDC_Handle->pTxData, (uint16_t) CDC_Handle->TxDataLength, CDC_Handle->DataItf.OutPipe, 1U);
		}
...
static void CDC_ProcessReception(USBH_HandleTypeDef *phost) {
	...
	case CDC_RECEIVE_DATA:
		...
		(void) USBH_BulkReceiveData(phost, CDC_Handle->pRxData, CDC_Handle->DataItf.InEpSize, CDC_Handle->DataItf.InPipe);
...
/**
 * @brief  The function informs user that line state have been changed
 *  @param  pdev: Selected device
 * @retval None
 */
